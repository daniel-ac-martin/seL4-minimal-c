...
CFLAGS ?= -g -DNDEBUG -ffreestanding -Wall -Wextra -fno-exceptions -std=gnu11 $(CFLAGS_$(arch))
...
build/$(arch)-$(plat)/src/roottask.elf: build/$(arch)-$(plat)/seL4/sel4runtime/libsel4runtime.a build/$(arch)-$(plat)/seL4/libsel4/libsel4.a build/$(arch)-$(plat)/seL4/musllibc/build-temp/lib/libc.a $(OBJ)
	mkdir -p $(@D)
	$(CC) $(CFLAGS) -v $(CLINK_FLAGS) \
		-Wl,-T deps/seL4/seL4_tools/cmake-tool/helpers/tls_rootserver.lds \
		build/$(arch)-$(plat)/seL4/lib/crt0.o \
		build/$(arch)-$(plat)/seL4/lib/crti.o \
		$(OBJ) \
		-Wl,--start-group \
		-lgcc \
		-lgcc_eh \
		build/$(arch)-$(plat)/seL4/sel4runtime/libsel4runtime.a \
		build/$(arch)-$(plat)/seL4/libsel4/libsel4.a \
		build/$(arch)-$(plat)/seL4/musllibc/build-temp/lib/libc.a \
		-Wl,--end-group \
		build/$(arch)-$(plat)/seL4/lib/crtn.o \
		-o $(@)
